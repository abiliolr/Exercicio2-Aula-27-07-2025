...
public class ReflexaoTabela {
	...
	// NOVO: retorna o nome da tabela anotação @Tabela
	public static String getTableName(SuperTabela<?> tab) {
    validarParametroTab(tab);
    Class<?> cls = tab.getClass();
    if (cls.isAnnotationPresent(Tabela.class)) {
        String n = cls.getAnnotation(Tabela.class).name();
        if (n != null && !n.trim().isEmpty()) return n;
    }
    return cls.getSimpleName().toLowerCase();
}

// NOVO: valida se todos os @Campo(isObrigatorio=true) estão preenchidos
	public static boolean isCamposObrigatoriosPreenchidos(SuperTabela<?> tab) {
    validarParametroTab(tab);
    Class<?> cls = tab.getClass();
    // varrer apenas os atributos declarados na classe concreta
    for (Field attr : cls.getDeclaredFields()) {
        if (attr.isAnnotationPresent(Campo.class)) {
            Campo cmp = attr.getAnnotation(Campo.class);
            if (cmp.isObrigatorio()) {
                Object valor = getValue(tab, attr);
                if (!isPreenchido(valor)) return false;
            }
        }
    }
    return true;
...
// NOVO: obter valor (getXxx)
	private static Object getValue(SuperTabela<?> tab, Field field) {
    String method = "get" + getUCFirst(field.getName());
    return invokeMethod(tab, method, null);
}

// NOVO: regra “se estar preenchido”
	private static boolean isPreenchido(Object v) {
